...
function loadFunctionWordDict() {
    return fetch("http://127.0.0.1:5000/get-dict")
        ...
        .then(data => {
            ...
            console.log("默认词典加载成功:", FUNCTION_WORD_DICT);
        })
        .catch(err => console.error("词典加载失败:", err));
}
...
function importFunctionWordDict(fileInput) {
    ...
    reader.onload = e => {
        try {
            FUNCTION_WORD_DICT = JSON.parse(e.target.result);
            mergeCustomWordsIntoDict();
            renderFunctionWordDict();
            compareTexts();
        } catch {
...
function importDoccanoJSONL(fileInput) {
    ...
    reader.onload = e => {
        ...
        FUNCTION_WORD_DICT = { ...newEntries, ...FUNCTION_WORD_DICT };
        renderFunctionWordDict();
        compareTexts();
...
/* ====================================================
   ...
   ==================================================== */
function addCustomLabel() {
    // 某些旧版 index.html 可能仍调用 addCustomLabel()
    // 这里直接转发到新的 addCustomCategory()
    return addCustomCategory();
}
